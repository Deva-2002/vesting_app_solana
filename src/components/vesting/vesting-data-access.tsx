...
export function useVestingProgramAccount({ account }: { account: PublicKey }) {
  ...
  mutationKey: ["vesting", "employee", { cluster, account }],
  mutationFn: ({ startTime, endTime, totalAmount, cliffTime, beneficiary }) =>
    program.methods
        .createEmployeeAccount(
        new BN(startTime),   // i64
        new BN(endTime),     // i64
        new BN(cliffTime),   // i64
        new BN(totalAmount), // u64
      )
      .accounts({
        beneficiary, // already a PublicKey
        vestingAccount: account, // probably also required
        // payer: program.provider.publicKey, // likely needed
        // systemProgram: SystemProgram.programId, // typical for PDAs
      })
      .rpc(),
  onSuccess: (tx) => {
    transactionToast(tx);
